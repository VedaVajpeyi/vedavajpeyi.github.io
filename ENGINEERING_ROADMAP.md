...
- **Status:** ✅ Complete. Commit `d1c57d0`. Shared asset `?v=` bumped to `20260715-2`.
  - **Real bug found and fixed along the way, not anticipated going in:** `<main>` carried a `data-nav-light` attribute (`page.js`'s nav/cursor light-mode switch, `[data-t="light"]` → `background: rgba(244,239,232,0.92)`) telling the nav to go light-mode while scrolled over that section — correct when the section was genuinely light-papered, now stale. First surfaced as an intermittent cream band behind the nav during verification; initially mistaken for a Lenis programmatic-scroll rendering artifact (a real, separate phenomenon also observed this session — `scrollIntoView`/`window.scrollTo` calls do desync Lenis's virtual scroll and produce transient blank/mis-rendered frames, unrelated to this bug) before being traced to the actual cause and fixed. Removed the attribute; confirmed via computed style that `nav[data-t]` stays `"dark"` across the full scroll range.
  - The pre-existing "writing-redesign.css's independent color palette" Deferred item is resolved by this task; removed from Deferred below.

**Phase 3 status: ✅ COMPLETE — all 8 tasks done.** Total across the phase: 78 files changed across all tasks (52 pages + `404.html` + `design.css` + `page.js` + `writing-redesign.css` + `work-scroll-morph.css` + `sitemap.xml`, some overlapping), 0 visual regressions, 0 console errors, chrome-sync clean throughout.
