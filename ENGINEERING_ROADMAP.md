...
- **Dead CSS from Task 9.17's homepage simplification.** `.marquee-wrap`/`.marquee-track` (and its `@keyframes`), `.work-sub`, `.writing-intro` in `design.css` now have zero remaining HTML references after the ticker/intro-paragraph removal. Small, safe, mechanical deletion — good candidate for a quick follow-up pass.
