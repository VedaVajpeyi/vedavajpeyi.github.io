...
- **Unify `writing-redesign.css`'s light "paper" theme with the site's dark theme.** Tried in Task 3.8 (commit `d1c57d0`): shipped, fully verified, zero regressions — then explicitly reverted (`99f1774`) after the owner reconsidered. The distinct fonts, palette, and rounded/shadowed cards are confirmed intentional. See Task 3.8 for the full detail. Don't re-propose without new instruction.
