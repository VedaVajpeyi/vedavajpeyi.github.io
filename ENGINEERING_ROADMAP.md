...
**Decision checkpoint — not scheduled as its own task:**
- Full mobile-taper alignment (≤600px) — `.hero-h1`'s three bespoke breakpoint tiers and its JS fit-calculation script are architecturally different from `.pg-h1`'s simple single-breakpoint clamp. Achieving full parity here means either replicating `.pg-h1`'s (much simpler) mobile behavior across all three tiers, or removing the JS mechanism and trusting pure CSS clamps not to overflow. Left as a follow-up, not pushed through without dedicated narrow-viewport testing.
