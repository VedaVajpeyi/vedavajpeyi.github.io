...
- **Three different bullet-marker mechanisms.** Found in Task 9.13's broader sweep for icon/glyph consistency: `.prose ul`'s native browser bullet (long-form essay/case-study content), `.sxp-list li::before`'s CSS-drawn dot (comparison/note components), `.srv-num-aside li::before`'s literal `·` character (services "for individuals" panel). Unlike the arrow fix, not clear-cut — the prose/component split could be legitimate content-type variation; the two compact-UI dot mechanisms (`.sxp-list` vs. `.srv-num-aside`) look more like unreconciled drift. Needs a look, not an assumption either way.
- **Eyebrow divider-tick width/color drift.** Found alongside the bullet-marker sweep. The short line after `.s-label`/`.pg-hero-kicker`/`.srv-close-kicker` (`design.css`) is 32px, solid `--accent`. The equivalent in `sociology-product-system.css` (`.socx-guide-hero-panel-label::after`, `.socx-guide-command-kicker::after`) is 28px and 34px respectively, at 55% opacity. Small, marginal — but the same pattern (one decoration, several unreconciled implementations) as the arrow issue, just lower-stakes.
