...
### Task 12.6 ✅ COMPLETE — done via Codex (this session hit a usage limit; owner switched tools), completed here
- **Objective:** Owner asked Codex to systematically tokenize hero-eyebrow spacing site-wide after noticing "almost each anchor page hero has different padding" — not another isolated value tweak.
- **Why this exists:** `#hero` (homepage), `.pg-hero` (About/Services/Contact/Sociology x Product/Work case studies/Writing essays/Topics/404), `.morph-hero` (Work listing page), and `.wr-hero` (Writing listing page) were four independently-maintained hero shells, each with its own literal padding values — Task 12.4/12.5 had only ever touched `.pg-hero`/`#hero`, not the other two.
- **What Codex did, verified against the actual committed/working-tree state rather than trusted from its own transcript:**
  - Added shared tokens to `design.css`: `--hero-shell-pad-top/-x/-bottom` (+ `-mobile` variants) and `--eyebrow-gap`/`--eyebrow-rule-length`.
  - Routed `.pg-hero`, `.morph-hero` (`work-scroll-morph.css`), and `.wr-hero` (`writing-redesign.css`) onto the shared padding tokens; routed `.pg-hero-kicker`/`.hero-kicker`/`.srv-close-kicker` onto `--eyebrow-gap`/`--eyebrow-rule-length`.
  - First push (`bd2946d`) left `#hero` still on its own literal values and index.html's inline `<style>` block still carrying homepage-only `#hero` padding overrides at three breakpoints — these were silently canceling out the new shared tokens. Caught this itself on a second pass (per its own transcript) and rewired `#hero` onto the same shared tokens, deleting the homepage-only override block (32 lines) from `index.html` — but hit a usage limit before verifying, committing, or pushing that second round.
- **What was still sitting uncommitted when this session picked it up:** the `#hero` token rewire and the 32-line homepage override deletion, confirmed correct on inspection and carried forward as part of this task rather than redone.
- **Files changed:** `design.css`, `work-scroll-morph.css`, `writing-redesign.css` (from Codex's committed work), `index.html` (from the uncommitted portion, verified and kept).
- **Status:** ✅ Complete, folded into this session's verification pass — see Task 12.7 for what was still wrong after this and why.

### Task 12.7 ✅ COMPLETE
- **Objective:** Owner reported home/about hero padding still looked different after Task 12.6, and separately that the Work/Writing listing-page hero titles were still rendering at the old, pre-Task-12.3 large size — asking specifically *why* a supposedly-tokenized value could still drift, and for a systemic fix, not another one-off.
- **Finding 1 — padding was actually already unified; the visible difference was a different mechanism:** Verified live: `.pg-hero`/`#hero` padding and `.pg-hero-kicker`/`.hero-kicker` margin were already byte-identical across Home/About/Services after Task 12.6. The remaining visible gap came from `justify-content: flex-end` (`#hero`/`.pg-hero`) and `align-items: flex-end` (`.morph-hero`/`.wr-hero`, row-direction equivalent) bottom-anchoring hero content inside a fixed-`min-height` box. Pages with less hero copy (About: a name + one line) leave more empty space above the content than pages with more (Services: headline + deck), and the homepage's box is taller still (`100vh` vs. `60vh`) — so the *eyebrow's on-screen position* varied by page even though the *padding value* didn't. Confirmed by measuring: Services' eyebrow sat exactly at `padding-top` (110px, no slack — content already filled the box); About's sat 140px lower (short content, bottom-anchored inside the same box, real empty space above it).
  - **Fix:** changed `flex-end` → `flex-start` (and `align-items` equivalent) on all four hero shells, so every hero's content — not just the eyebrow — now sits exactly `padding-top` from the top of its box on every page, regardless of content length or `min-height`. Verified: Home/About/Services all now measure the eyebrow at exactly `110px`, previously `277px`/`250px`/`110px` respectively.
- **Finding 2 — the Work/Writing listing-page hero titles were never actually wired to the token:** `.morph-title .wp` (`work-scroll-morph.css`) and `.wr-title .wp` (`writing-redesign.css`) both had `font-size: clamp(2.8rem, 7vw, 7rem)` hand-copied as a literal instead of `var(--type-h1-page)`. That literal happened to equal the token's value at the time either was written — so it looked correct for months — but when Task 12.3 changed the token to its current `clamp(2.8rem, 5.25vw, 5.25rem)`, these two silently kept rendering the old 112px cap instead of following it down to 84px, because they were never actually consuming the token in the first place. A repo-wide grep for every other CSS file confirmed these were the only two instances of this exact failure mode (a hardcoded copy of an old token value) anywhere in the codebase.
  - **Fix:** both repointed to `font-size: var(--type-h1-page)` directly.
  - **Systemic prevention, since this is explicitly a "make sure it never happens again" ask:** there's no build step or linter in this repo to catch a hardcoded literal automatically, so the only real guardrail available is making the mistake harder to make unknowingly. Added a comment directly at `--type-h1-page`'s definition in `design.css` stating the rule explicitly (every consumer must reference the token, never copy its value), listing every known consumer by name, and instructing that changing this token requires grepping every listed consumer beforehand and grepping for a literal match afterward to confirm nothing was missed. This won't stop a future editor from making the same mistake, but it puts the exact failure mode and its fix in the one place someone editing this token will actually see.
- **Files changed:** `design.css` (`#hero`, `.pg-hero`, token comment), `work-scroll-morph.css` (`.morph-hero`, `.morph-title .wp`), `writing-redesign.css` (`.wr-hero`, `.wr-title .wp`).
- **Verification checklist:** Brace-balance clean on all 3 CSS files; `sync-chrome.mjs` clean; `bump-asset-version.mjs 20260721-6` run, full 62-page 200 sweep; live computed-style checks confirmed `.morph-title`/`.wr-title` both resolve to `84px` (matching `.pg-h1`/`.hero-h1`/`.contact-h2` exactly); confirmed Home/About/Services eyebrows all sit at exactly `110px` from their box's top; visual screenshots of About (short content), Work listing, and Writing listing confirm nothing broke — no awkward floating content, no overlap, no orphaned whitespace at the bottom that reads as a mistake.
- **Success criteria:** One eyebrow/hero-shell behavior across the entire site, driven by one set of tokens with no remaining hardcoded copies anywhere; the Work/Writing title-size regression fixed and its root cause (a literal that isn't wired to its token) eliminated everywhere it existed, not just patched where it was noticed.
- **Rollback plan:** Six small, independent rule changes across three files — each trivially revertible.
- **Estimated risk:** Low — the `flex-start` change is a real behavior change (verified with screenshots, not just computed style, given it affects overall hero composition on shorter-content pages); the token rewiring is a pure value-preserving fix (both selectors now resolve to the exact value the token already defines).
- **Status:** ✅ Complete. Commit pending. Shared asset `?v=` bumped to `20260721-6`.

### Task 12.8 ✅ COMPLETE
- **Objective:** Owner asked for more than the Task 12.7 one-off fix — a systematic sweep for every other value across the site that should be referencing an existing token but is instead a stray hardcoded literal, plus a guardrail so this class of bug (a token changes, a hand-copied literal silently doesn't follow it) can't recur unnoticed.
- **Method:** For every token in `design.css`'s `:root` with a plausible "same value, different property, hand-copied" risk (typography scale, the RGB-triplet color tokens, the brand-new hero-shell/eyebrow tokens), grepped every one of the 6 live CSS files for a literal match outside the token's own definition. This is exactly what caught `.morph-title`/`.wr-title` in Task 12.7 — the same method, just run against every token instead of the one that had already broken.
- **Findings and fixes, typography:**
  - `.contact-link-value`, `.srv-also a` (`design.css`) had `font-size: clamp(1.15rem, 2vw, 1.65rem)` hand-copied — byte-identical to `--type-card-compact`, and already sharing that role's exact weight/line-height/letter-spacing recipe (the same "Compact Card Title" pattern Task 9.22 tokenized for `.srv-num-main h3`/`.contact-service-name`, just missed for these two). Repointed to `var(--type-card-compact)`.
- **Findings and fixes, color (a much larger gap than expected):** Task 4.1 (this engagement's original RGB-tokenization pass) only ever covered `design.css` — the 4 satellite stylesheets (`sxp-essays.css`, `sociology-product.css`, `sociology-product-system.css`, `writing-redesign.css`) were never swept and had accumulated **40 stray literal RGB triplets** matching existing color tokens exactly, plus 2 more inside `design.css` itself that had been missed the first time round:
  - `rgba(181,146,90, X)` (= `--accent-rgb`): 34 occurrences across `sxp-essays.css` (5), `sociology-product.css` (3), `sociology-product-system.css` (26).
  - `rgba(240,235,227, X)` (= `--text-d-rgb`): 6 occurrences across `design.css` (2, missed by the original Task 4.1 sweep), `sociology-product.css` (1), `sociology-product-system.css` (3), `writing-redesign.css` (1, already fixed as part of Task 12.2's `.hero-sub` work but the sibling `.wr-lede` instance had the same literal and was still stray).
  - `rgba(13,11,9, X)` (= `--dark`'s RGB): **10 occurrences with no token to reference at all** — `--dark` never had an `-rgb` sibling the way `--accent`/`--text-d`/`--muted-d`/`--muted-l` do, despite being used in `rgba()` context in `design.css` itself (2 places) and `sociology-product-system.css` (8 places). Added `--dark-rgb: 13,11,9;` and repointed all 10.
  - All converted via scripted regex substitution (same method as Task 4.1: replace only the `rgba(R,G,B,` prefix, leaving the alpha value and everything after untouched) — every changed line's before/after pair reviewed to confirm the alpha value survived exactly.
  - One more hardcoded hex found and fixed along the way: `sociology-product-system.css`'s hero gradient had `#0d0b09` (exactly `--dark`) as a literal gradient stop, sitting next to a second, genuinely unique stop color (`#17120d`, left alone — doesn't match anything). Repointed the first stop to `var(--dark)`.
- **Finding and fix, a second token system entirely:** the writing sub-system's own local `--wr-*` tokens (`writing-redesign.css`) had the identical gap at their own scale — `--wr-ink` (`#15221d`) had no `-rgb` sibling, and **14 occurrences** of its literal RGB equivalent (`21,34,29`) were scattered through the file at 10 different alpha values, including inside components added after `--wr-rule` (the one `--wr-*` color that *was* already tokenized as an rgba) was established. Added `--wr-ink-rgb: 21,34,29;` and repointed all 14. This directly answers the "other token systems" half of the ask — `--wr-*` is a real, separate, self-contained token system with the exact same failure mode as the sitewide one.
- **Checked and confirmed clean, not just assumed:** the hero-shell/eyebrow tokens from Task 12.6 (`--eyebrow-rule-length`, `--eyebrow-gap`) — grepped for any other `32px` divider-tick or `1.5rem`/`24px` kicker-adjacent margin-bottom that should be using them; none found, adoption is already complete. `--type-display-2`, `--type-h2`, `--type-h3`, `--type-card-featured` — grepped for literal clamp() duplicates in all 6 files; only the two already-fixed `--type-h1-page` instances (Task 12.7) and the one `--type-card-compact` instance above turned up anywhere.
- **Guardrail:** beyond Task 12.7's warning comment on `--type-h1-page` specifically, this task's real prevention mechanism is the sweep itself becoming a documented, repeatable method — grep every token's literal value across all 6 CSS files, not just `design.css` — now written down here so the next time a token changes, checking for stray copies isn't a one-off improvisation.
- **Files changed:** `design.css` (2 fixes + 1 new token), `sxp-essays.css` (5 fixes), `sociology-product.css` (4 fixes), `sociology-product-system.css` (38 fixes: 37 RGB + 1 hex), `writing-redesign.css` (15 fixes + 1 new token).
- **Verification checklist:** Brace-balance clean on all 5 touched files; every substitution's before/after line pair reviewed inline during the scripted run to confirm alpha values preserved exactly; `sync-chrome.mjs` clean; `bump-asset-version.mjs 20260721-7` run, full 62-page 200 sweep; live console-error check (clean) and computed-style spot-checks on the two most heavily-edited pages (`sociology-product-field-guide.html`, `writing/index.html`) confirmed colors render byte-identical to their pre-refactor values (e.g. `.wr-section-kicker` color `rgba(21, 34, 29, 0.56)`, `.wr-path-card` border `rgba(21, 34, 29, 0.18)` — both unchanged).
- **Success criteria:** Zero remaining stray literal duplicates of any current token's value across all 6 live CSS files (typography, color, hero-shell/eyebrow); zero visual or behavioral change anywhere; two previously-missing tokens (`--dark-rgb`, `--wr-ink-rgb`) added so the color system has no more gaps of this shape.
- **Rollback plan:** Five independent per-file reverts.
- **Estimated risk:** Low-medium going in (55 total call sites across 5 files) — mitigated by scripted (not manual) substitution eliminating transcription risk, per-line before/after review, and live computed-style verification rather than assuming the mechanical change was safe.
- **Status:** ✅ Complete. Commit pending. Shared asset `?v=` bumped to `20260721-7`.

### Task 12.9 ✅ COMPLETE
- **Objective:** Owner asked to confirm two heading tiers are properly tokenized site-wide — Title 1 (the 6 hero/page-arrival headlines) and Title 2 (the 6 homepage section headings) — then, having noticed About's "My Story"/"My Approach"/"What I Believe"/"Experience" and Sociology x Product's "The blind spot in product management" render at inconsistent sizes, asked for the correct role taxonomy to be established and applied site-wide, not fixed page by page.
- **Title 1, confirmed tokenized — all 6 share `var(--type-h1-page)`:** `.hero-h1` (homepage), `.pg-h1` (About/Services/Sociology x Product), `.morph-title .wp` (Work listing), `.wr-title .wp` (Writing listing) — the last two only as of Task 12.7's fix. Live-verified: all compute to the same value at the same viewport.
- **Title 2, confirmed tokenized with one real exception:** `.about-h2`/`.work-h2`/`.project-h2`/`.writing-h2`/`.services-h2` all share `var(--type-display-2)` (65.6px cap), confirmed identical. **`.contact-h2` does not** — it currently uses `var(--type-h1-page)` (84px cap, matching Title 1) rather than `--type-display-2`, per Task 9.16/12.3's "bookend" decision (deliberately echo the hero's scale, not its Title-2 siblings). The owner's Task 12.9 request lists `.contact-h2` alongside its five Title-2 siblings without carving out an exception, which directly conflicts with that standing decision. Not resolved unilaterally — flagged back to the owner rather than silently overriding a twice-confirmed prior decision a third time.
- **Root cause of the reported inconsistency, found by building a complete taxonomy first:** the site has two genuinely distinct "subsection heading" roles that look similar but aren't the same thing — **Display Medium** (`--type-display-2`, 65.6px, homepage-only oversized section dividers) and **Section Title** (`--type-h2`, 35px, in-page content subsection headings via `.prose h2`). About's 5 subheads and 5 of Sociology x Product's 6 examples were already correctly on Section Title and already matched each other exactly (verified live, all 35px) — not the bug. The real bug was narrower: `.socx-use-intro h2` (Sociology x Product's "Start here. Then take it into the room." intro block) had Section Title's exact recipe hand-copied as a bespoke `clamp(1.5rem, 2.4vw, 2.1rem)` literal instead of referencing `var(--type-h2)` — rendering 1.4px smaller than its five siblings on the same page. Fixed: repointed to the token, and its slightly-off `line-height` (1.1 vs. the role's `1.15`) corrected to match at the same time.
- **Full site-wide sweep performed before concluding this was the only fix needed:** catalogued every CSS rule targeting a bare `h2`/`h3` across all 6 stylesheets (21 selectors). Beyond the one fix above, everything else sorted cleanly into either Section Title/Subsection (already correct) or one of these confirmed-legitimate, deliberately separate systems, left untouched:
  - Card titles inside diagrams/comparison components (`.sxp-network-card h3`, `.sxp-matrix-card h3`, `.sxp-flow-card h3`, `.socx-model-header h3`, `.socx-proof-item h3`, `.socx-path-card h3`) — small illustrative card labels inside charts/tables, not page subsection headings; same category as the already-Deferred "card component consolidation" item, not a heading-hierarchy bug.
  - The Field Guide's own sheet/case/law headings (`.socx-guide-sheet h2/h3`, `.socx-guide-case h3`, `.socx-guide-law h3`, `.socx-guide-cta-card h3`) and the Writing sub-brand's own section/close headings (`.wr-section-head h2`, `.wr-close-copy h2`) — both confirmed-intentional distinct visual systems (print-like field guide; separate typeface/palette writing index), established and reconfirmed multiple times earlier in this engagement (Task 3.8/3.9 specifically).
  - Established, already-tokenized card-title roles: `.list-item h2`/`.ci-h3`/`.essay-feat h3` (Featured Card Title), `.ei h3`/`.srv-num-main h3`/`.srv-ind-item h3`/`.value-item h3` (Compact Card Title).
- **Files changed:** `sociology-product.css` only.
- **Verification checklist:** Brace-balance clean; `sync-chrome.mjs` clean; `bump-asset-version.mjs 20260721-7` (no-op, already current from Task 12.8's run moments earlier); full 62-page 200 sweep; live computed-style check on `sociology-product.html` confirmed all 6 `h2`s on the page now render at identical `35px`, matching About's 5 subheads exactly.
- **Success criteria:** Every genuine Section Title-role heading site-wide renders at one consistent size; every heading that looks similar but serves a different, deliberately-distinct role is identified and left alone with a documented reason, not guessed at.
- **Rollback plan:** Single-rule revert, one file.
- **Estimated risk:** None realized.
- **Status:** ✅ Complete. Commit pending. Shared asset `?v=` bumped to `20260721-7`.

**Resolved in Task 12.10:** `.contact-h2` moved to Title 2 (Display Medium), matching its five siblings. See Task 12.10 and the standing rule it establishes.

### Task 12.10 ✅ COMPLETE
- **Objective:** Owner gave a standing rule, not a one-off answer to Task 12.9's open checkpoint: every recurring role maps to exactly one token, applied everywhere, with no bespoke exceptions — and asked for hero titles specifically to be confirmed unified across every page on the site, not just the anchor pages already checked, without further round-trips.
- **`.contact-h2` resolved per the standing rule:** moved off `--type-h1-page` (the Task 9.16/12.3 "bookend" exception) onto `--type-display-2`, matching `.about-h2`/`.work-h2`/`.project-h2`/`.writing-h2`/`.services-h2` exactly (`font-size`, and `letter-spacing` corrected from `-0.03em` to the role's `-0.025em` at the same time). Its now-orphaned `960px` mobile override (`clamp(2.5rem, 7.5vw, 3.75rem)`, a value none of its 5 siblings had) deleted — it now uses `--type-display-2`'s own fluid scaling continuously, same as its siblings, rather than a bespoke breakpoint tier. `--type-display-2-close` (already retired in Task 12.3) confirmed to have zero remaining references anywhere.
- **Hero-title unification confirmed complete, sitewide, before concluding rather than assumed:** enumerated every page using `.pg-h1` (49 of them: About, 7 Services pages, Contact, 404, Books & Beds, Sociology x Product, the Field Guide, 9 Topics pages, 5 Work case studies, 23 Writing essays) and grepped each individually for any page-level override of `.pg-h1`'s font-size — zero found on any page. Combined with `.hero-h1` (home), `.morph-title .wp` (Work listing), and `.wr-title .wp` (Writing listing, fixed in Task 12.7), all 52 hero-title instances on the site share `var(--type-h1-page)` with no exceptions. This was already true before this task for 51 of the 52 — this task's contribution was verifying and documenting it, not fixing new instances.
- **Token documentation updated to match:** `--type-h1-page`'s comment now lists all 52 real consumers by category instead of a partial list, and states the "confirmed as of Task 12.10, zero page-level overrides anywhere" finding directly, so a future editor doesn't have to re-derive this from scratch. `--type-display-2`'s comment updated to explicitly list all 6 consumers including `.contact-h2` and state "no exceptions."
- **Files changed:** `assets/css/design.css` only (`.contact-h2` rule, its mobile override removed, both token comments).
- **Verification checklist:** Brace-balance (479/479); `sync-chrome.mjs` clean; `bump-asset-version.mjs 20260721-8` run, full 62-page 200 sweep; live computed-style checks confirmed `.contact-h2` now renders at `65.6px` matching all 5 siblings exactly (was `84px`, matching the hero); spot-checked `.pg-h1` on a Work case study and a Writing essay (two of the 49 `.pg-h1` consumers not previously spot-checked individually) — both `84px`, matching the hero and every other page.
- **Success criteria:** Zero bespoke-exception headings remain in the Title 1/Title 2 system; every hero title on every page confirmed, not assumed, to share one token.
- **Rollback plan:** Single-file revert.
- **Estimated risk:** None realized.
- **Status:** ✅ Complete. Commit pending. Shared asset `?v=` bumped to `20260721-8`.
